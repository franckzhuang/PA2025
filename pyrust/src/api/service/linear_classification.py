...
def train_linear_classification(config, mongo_collection, job_id):
    ...
    experiment_config = {
        ...
        "real_images_path": config.get("real_images_path",  str(base_path / "data/real")),
    ...
    try:
        ...
        print(f"[{datetime.utcnow()}][{job_id}] Status: RUNNING | Config: {experiment_config}")
        ...
        print(f"[{datetime.utcnow()}][{job_id}] Loading dataset...")
        ...
        if total_images < 2:
            print(f"[{datetime.utcnow()}][{job_id}] FAILURE: Not enough images loaded (real: {len_real}, ai: {len_ai})")
            mongo_collection.update_one(
                {"job_id": job_id},
                {"$set": {
        ...
        print(f"[{datetime.utcnow()}][{job_id}] Starting training ({len(data['X_train'])} samples)...")
        ...
        print(f"[{datetime.utcnow()}][{job_id}] Success: Train acc={train_accuracy:.2f}%, Test acc={test_accuracy:.2f}%")
        mongo_collection.update_one(
            {"job_id": job_id},
            {"$set": {
        ...
        print(f"[{datetime.utcnow()}][{job_id}] ERROR: {str(e)}")
        mongo_collection.update_one(
            {"job_id": job_id},
            {"$set": {
                "status": Status.FAILURE.value,
